...
class NeuralNetwork:
    ...
    def train(
        ...
        for _e in range(epochs):
            ...
            r = self._backward(targets, output)
    ...
    def _backward(
        self, target_data: NDArray[np.float64], model_output: NDArray[np.float64]
    ) -> None:
        """
        Backward Propagation to calculate gradients
        """
        # Calculate deltas for output layer
        delta_next_layer = model_output - target_data

        # Backpropagate through the remaining hidden layers
        for i in range(len(self.layers) - 1, 0, -1):
            delta_next_layer = self.layers[i].backward(delta_next_layer, self.layers[i].weights)
